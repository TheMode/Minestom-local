...
public sealed interface Chunk extends Viewable, Tickable, Taggable, Snapshotable permits ChunkImpl {
    ...
    long NO_FLAGS = 0L;
    long LIGHTING_FLAG = 1L;
    long READONLY_FLAG = 1L << 1;
    long GENERATE_FLAG = 1L << 2;

    static Chunk chunk(Instance instance, int chunkX, int chunkZ, @MagicConstant(flagsFromClass = Chunk.class) long flags) {
        final boolean lighting = (flags & LIGHTING_FLAG) != 0;
        final boolean readOnly = (flags & READONLY_FLAG) != 0;
        final boolean generate = (flags & GENERATE_FLAG) != 0;
        ...
        Viewable viewable = instance.getEntityTracker().viewable(shared, chunkX, chunkZ);
        return new ChunkImpl(instance, chunkX, chunkZ, instance.getCachedDimensionType(),
                lighting, generate, readOnly, viewable);
    }
    ...
    static Chunk chunkLight(Instance instance, int chunkX, int chunkZ) {
        return chunk(instance, chunkX, chunkZ, LIGHTING_FLAG | GENERATE_FLAG);
    }
