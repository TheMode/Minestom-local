...
record PainterImpl(List<Instruction> instructions) implements Painter {
    ...
    static final class WorldImpl implements ReadableWorld {
        ...
        public @UnknownNullability Block getBlock(int x, int y, int z, Condition condition) {
            final Palette palette = sectionAt(instructions, globalToSection(x), globalToSection(y), globalToSection(z));
            final int stateId = palette.get(globalToSectionRelative(x), globalToSectionRelative(y), globalToSectionRelative(z));
        ...
        public void cuboid(Point min, Point max, Block block) {
            append(new Instruction.Cuboid(min.asVec(), max.asVec(), block));
        }
    ...
    static void applyInstruction(int sectionX, int sectionY, int sectionZ, Palette palette,
        ...
        switch (instruction) {
            case Instruction.SetBlock setBlock -> {
                ...
                final int localX = globalToSectionRelative(absX);
                final int localY = globalToSectionRelative(absY);
                final int localZ = globalToSectionRelative(absZ);
    ...
    static boolean sectionRelevant(Instruction instruction, int sectionX, int sectionY, int sectionZ, Point offset) {
        return switch (instruction) {
            case Instruction.SetBlock setBlock -> globalToSection(setBlock.x() + offset.blockX()) == sectionX &&
                    globalToSection(setBlock.y() + offset.blockY()) == sectionY &&
                    globalToSection(setBlock.z() + offset.blockZ()) == sectionZ;
            case Instruction.Cuboid cuboid -> {
    ...
    static boolean sectionInBound(Vec min, Vec max, int sectionX, int sectionY, int sectionZ) {
        final int minX = globalToSection(min.blockX());
        final int minY = globalToSection(min.blockY());
        final int minZ = globalToSection(min.blockZ());

        final int maxX = globalToSection(max.blockX());
        final int maxY = globalToSection(max.blockY());
        final int maxZ = globalToSection(max.blockZ());
